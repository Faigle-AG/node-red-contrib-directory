    ...
    function DirectoryActionNode(config) {
        ...
        node.on("input", function(msg, send, done) {
            try {
                ...
                const targetRaw     = node.dynamic ? (msg.file && msg.file.path) : RED.util.evaluateNodeProperty(node.target, node.targetType, node, msg);

                if (!targetRaw)
                    throw new Error("Target directory path is missing");
